...
// Add value to a node that has space
...
// Split the node when it exceeds MAX
...
// Recursive function to set value in node
...
void searching(int val, int *pos, struct btreeNode *myNode) {
    if (!myNode) {
        printf("Value %d not found in Btree \n", val);
        return;
    }
    ...
    while (1) {
        ...
        switch (ch) {
            ...
            case 2:
                ...
                scanf("%d", &val);
                searching(val, &ch, root); // Note: using &ch as a temp variable for pos
                break;
            case 3:
                printf("\n The traversal of the given element is: ");
                traversal(root);
                break;
            case 4:
                exit(0);
